...
class App:
    def __init__(self):
        ...
        # Caminho do arquivo salvo na mesma pasta que 'pagadores.py' e 'usuarios_pagadores.py'
        self.caminho_arquivo = os.path.join(self.localizar_diretorio_base(), "login_imap.txt")
    ...
    def localizar_diretorio_base(self):
        """Localiza o diretório onde estão 'pagadores.py' e 'usuarios_pagadores.py'."""
        for arquivo in ["pagadores.py", "usuarios_pagadores.py"]:
            caminho = os.path.join(os.path.dirname(os.path.realpath(__file__)), arquivo)
            if os.path.exists(caminho):
                return os.path.dirname(caminho)
        
        raise FileNotFoundError("Os arquivos 'pagadores.py' e 'usuarios_pagadores.py' não foram encontrados.")
